...
public class PvPTutorial implements Screen {
    RPS game;
    ...
    // Constructor
    public PvPTutorial(RPS game, ArrayList<Integer> scores, int player1, int player2) {
        this.game = game;
        batch = new SpriteBatch();
        background = new Texture(Gdx.files.internal("PvPTutor.png"));
    ...
    public void render(float delta) {
        ...
        if((Gdx.input.getX()>=Start.x && Gdx.input.getX()<=Start.x+Start.width)&&(Gdx.graphics.getHeight()-Gdx.input.getY()>=Start.y && Gdx.graphics.getHeight()-Gdx.input.getY()<=Start.y+Start.height)) {
            ...
            if(Gdx.input.isTouched()) {
                game.setScreen(new PvP(game, highscores, p1Highscores, p2Highscores));
            }
        } else {
            startBtn = new Texture(Gdx.files.internal("Start.png"));
        }
    ...
    // disposes SpriteBatch and images when the game is closed or screen is changed
    @Override
    public void dispose() {
        batch.dispose();
        background.dispose();
